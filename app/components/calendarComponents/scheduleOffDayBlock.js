...
/**
 * ScheduleOffDayBlock Component
 * 
 * This component represents an off day schedule block with drag-and-drop functionality.
 ...
 * @returns {JSX.Element} The ScheduleOffDayBlock component
 */
const ScheduleOffDayBlock = ({
  id,
  ...
  return (
    <div ref={(node) => drag(drop(node))} className="relative" style={{ opacity: isDragging ? 0.5 : 1 }}>
      <button
        ...
        className="inline-block px-4 py-2 text-yellow-600 font-bold cursor-pointer relative outline-none rounded-lg text-lg h-[80px] flex flex-col items-start justify-center pl-4 transition-all hover:border-yellow-800"
        style={{
          backgroundColor: 'rgba(240, 230, 140, 0.5)', // Less bright translucent yellow
          border: '2px solid #C8B900', // Darker yellow border
      ...
      >
        ...
        {isMultiMonth && (
          <span className="absolute right-0 flex items-center h-full pr-2">
            <HiOutlineChevronDoubleRight className="text-yellow-600 text-3xl" />
          </span>
        )}
      </button>
      ...
      {isPopupOpen && (
        <div className="absolute text-black left-0 top-[60px] w-[280px] bg-white border border-white p-4 shadow-md z-50 rounded-lg">
          <div className="flex justify-between items-start">
            <div>
              <div className="font-bold text-xl text-black">Off <span className="inline-block w-3 h-3 rounded-full bg-yellow-400 ml-2"></span></div>
          ...
          {!isEditing && (
            <>
              <div className="mt-4 text-black font-bold">Off Till:</div>
...
export default ScheduleOffDayBlock;
